...
public class Player extends GameObject {
    ...
    private void initRigibody2D() {
        rigibody2d = new Rigibody2D(1f, new Vector2D(0, 0), position, 1);
        rigibody2d.dragCoefficient = 0.2f;
    }
    ...
    private void drawPlayer(Graphics2D g2d) {
        ...
        Vector2D velocity = rigibody2d.velocity;
    ...
    private void updatePos() {
        ...
        rigibody2d.applyGravity(Game.getInstance().GRAVITY.multiply(speedForce));
        rigibody2d.applyDrag();
        rigibody2d.update();
        ...
        if (position.x == minX || position.x == maxX - width) {
            rigibody2d.velocity = rigibody2d.velocity.multiply(-1);
        }

        rigibody2d.position = position;
    ...
    public void mouseClickForce() {
        if (isAlive) {
            ...
            rigibody2d.applyForce(forceToMouseClicked.multiply(speedForce));
        }
    }
